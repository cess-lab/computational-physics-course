...
Every lecture or practical-brief slide deck, and every material slide-deck revision, uses the installed `codex-ppt` skill with its built-in **Teaching Courseware** style. The canonical deck is an image-based 16:9 PPTX with one complete generated image per slide and English presenter notes embedded from `speech.md`. Follow the skill's outline, style, backend, sample-approval, slide-job, state-recording, QA, and assembly gates. Use the built-in image-generation backend unless the lecturer explicitly approves CLI/API fallback. Preserve equations, MATLAB code, units, numerical values, and computed evidence as strict content facts. For lecture-slide graphs, use MATLAB figures as reference-only inputs and default to a complete built-in ImageGen redraw of the graph, labels, annotations, and slide together; then compare the final graph against the retained MATLAB source during QA rather than overlaying the MATLAB raster.

Do not route a current PHY4605 slide deck through the retired editable-first `Presentations`/`@oai/artifact-tool` system or impose its Nunito/IBM Plex Mono typography contract. Preserve that system only as hidden historical evidence.
...
**Cross-material visual rule:** Where a scientific illustration removes a novice-level representation barrier, embed an approved ImageGen visual directly in the learning note and/or MATLAB Live Script rather than linking externally. Reuse a retained strict source visual from a Codex-PPT project only when the scientific mapping is identical; do not crop a complete slide merely to recover an illustration. Keep MATLAB-generated plots as strict source/reference evidence: for lecture-slide visuals, pass them to ImageGen as reference-only inputs for a cohesive full-slide redraw instead of pasting or overlaying the raster; for learning notes and MATLAB Live Scripts, embed the MATLAB-generated plot where specified. Record the source comparison and a reason in hidden QA evidence when an illustration or redraw is not justified.

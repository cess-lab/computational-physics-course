...
const ARRAY_MOTION = path.join(SYSTEM, "assets/array-sampled-motion-matte-v2.png");
const LAUNCH_BOUNDARY = path.join(SYSTEM, "assets/launch-boundary-condition-matte.png");
const EQUATION_MOTION = path.join(SYSTEM, "assets/equation-motion-vertical-matte-v2.png");
...
function addEquationAsset(slide, blob, position, { alt, tex, name, assetKey, foreground = C.navy, fit = "contain" } = {}) {
  ...
  entries.push({
    assetKey: key,
    basePath: `.agent/lecture-slide-system/assets/equations/${base}`,
    tex: tex || "",
    foreground,
...
async function main() {
  ...
  // Slide 1 — opening system proof.
  {
    ...
    addText(slide, "Editable PowerPoint master proof", { left: 115, top: 905, width: 620, height: 34 }, { fontSize: 24, color: C.muted });
  ...
  // Slide 6 — pseudocode and code hierarchy.
  {
    ...
    addText(slide, "The algorithm appears before the MATLAB syntax", { left: 115, top: 165, width: 1580, height: 50 }, { fontSize: 30, color: C.charcoal }, "subtitle-T1-slide6");
  ...
  // Slide 9 — system recap and approval checkpoint.
  {
    const slide = presentation.slides.add({ layout: "Content T1" });
    slide.background.fill = C.white;
    addTitle(slide, "The design system keeps every week coherent", "T2", "Approve this proof before the Week 1 teaching deck is rebuilt");

    // Arrows first so they stay behind the nodes.
    [425, 820, 1215].forEach((x) => addArrow(slide, { x, y: 445 }, { x: x + 80, y: 445 }, C.rule, 3));
    const nodes = [
      { x: 115, no: "01", head: "Typography", body: "Nunito titles and body\nIBM Plex Mono code", fill: C.paleBlue, color: C.blue },
      { x: 510, no: "02", head: "Structure", body: "T1, T2, and T3 title rules\nlocked content zones", fill: C.paleNeutral, color: C.navy },
      { x: 905, no: "03", head: "Visuals", body: "editable scientific graphics\napproved generated assets", fill: C.paleGold, color: C.gold },
      { x: 1300, no: "04", head: "QA", body: "render, inspect, and check\nfonts, titles, and overflow", fill: C.paleValid, color: C.green },
    ];
    nodes.forEach((node) => {
    ...
    addText(slide, "Review focus: title character, spacing, component shapes, chart treatment, and visual balance", { left: 160, top: 774, width: 1485, height: 70 }, { fontSize: 32, bold: true, color: C.navy, alignment: "center" });
    addSlideNumber(slide, 9);
    addNotes(slide, ["PHY4605 Lecture Slide Deck Design Specification", "PHY4605 editable master proof review checkpoint"]);
  }

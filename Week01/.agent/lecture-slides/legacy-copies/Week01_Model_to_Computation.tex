...
This note is designed to support a two-hour lecture and a short practical activity. Read the physical question first, make a prediction, and only then run the MATLAB experiment. The purpose is not to memorise a command sequence; it is to make the model, numerical method, and validation evidence visible.
...
\begin{itemize}[nosep,leftmargin=5mm]
  ...
  \item the \textbf{Editor/Live Editor} stores a repeatable script with text, code, and output;
...
Fresh run & Start with \texttt{clearvars; close all;} so old Workspace values cannot hide a mistake.\\ \bottomrule
\end{tabular}
\end{center}

\textbf{Mini warm-up: units and expressions.} The first practical contains short scalar problems. For example, convert a tank volume of 2 gallons and 4 pints to litres using 8 pints per gallon and 1.76 pints per litre:
\begin{lstlisting}[style=matlabstyle]
gallons = 2;
pints = 4;
pintsPerGallon = 8;
pintsPerLitre = 1.76;
volumeLitres = (gallons*pintsPerGallon + pints)/pintsPerLitre;
fprintf('Volume = %.3f L\\n', volumeLitres)
\end{lstlisting}
The result is approximately $11.364\,\mathrm{L}$. The important habits are to name the units, show the conversion, and inspect whether the magnitude is plausible.
...
plot(t_h,N/N0,'o-',t_h,N_exact,'k-','LineWidth',1.5)
...
The plot changes between runs & Run after \texttt{clearvars}; do not use old values.\\
...
\begin{aibox}
\textbf{AI with responsibility.} An AI assistant may help explain an Euler update or suggest MATLAB syntax. It cannot establish that your result is physically valid. Before using AI-assisted code, check the units, reproduce a limiting case, compare with $N_{\mathrm{exact}}$, and state which test you performed.
\end{aibox}
...
\begin{keybox}
\textbf{Preparation for the practical.} Bring a MATLAB script or Live Script that can simulate decay for a chosen $\Delta t$. Be ready to show one plot, one limiting-case check, and one sentence explaining whether your time step is adequate.
\end{keybox}
\end{document}

...
Status: split-slide revision requested in the lecturer's review comments on 2026-09-08. The current student-facing deck is preserved at `qa/Lecture_Slides_Week02.before-split-comments.pptx` and will not be replaced until the fresh sample, delegated generation, assembly, and QA gates pass.

Audience: Year-2 physics students who know familiar analytic physics but have uncertain retained MATLAB foundations.

Teaching goal: make the order of a repeated calculation visible, trace a loop, complete one storage action, change one parameter, diagnose a defect from evidence, and transfer the same reasoning to a supplied radioactive-decay model. Keep the debugging workflow and the detailed numerical/physical evidence on separate slides so each teaching point remains readable.
...
- Key points: Record the symptom; locate the line and first bad pass; compare the code with the pseudocode or model; change one item; rerun; validate numerically and physically.
- Visual idea: A spacious six-step diagnostic workflow from symptom -> first bad pass -> model/pseudocode -> one change -> rerun -> two checks. Keep the workflow itself as the only main content.
...
- Key points: Use the first-bad-pass table to locate where the output changes; compare the implemented line with the physical model; verify a known value such as `y(0.5 s) = 8.77375 m`; verify that the motion rises to one maximum and then falls.
- Visual idea: A readable three-panel evidence slide: first-bad-pass trace table on the left, correct-versus-wrong-sign code in the centre, and two compact validation checks on the right. Do not repeat the six-step workflow.
- Layout role and intent: Evidence / repair confirmation. Give the workflow's final checks enough space to be inspected rather than compressing them under the workflow.
- Required images: None. Exact table values, code fragments, signs, and units are strict slide text.
...
- Layout role and intent: Validation / limitation. Show what one check proves, what it misses, and why physical interpretation remains part of correctness after the evidence check.
...
- Debugging evidence: for the wrong positive-gravity-sign example, `y_m(1) = 0.00000 m`, `y_m(2) = 11.22625 m` at `t_s(2) = 0.5 s`, and `y_m(3) = 24.90500 m` at `t_s(3) = 1.0 s`; the correct first nonzero value is `y(0.5 s) = 8.77375 m`.
...
5. Inspect every final slide image at full size, verify the exact numerical/equation/code facts, embed presenter notes, assemble a 15-slide PPTX, and retain the pre-split deck backup.

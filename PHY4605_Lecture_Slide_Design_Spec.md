...
- **Course master title schema:** every PHY4605 weekly deck must use the same visual-equivalent condensed title face and weight, rather than merely a broadly similar sans-serif. Use the locked title and subtitle treatment on every slide, including slides with tables, code, or a long title. Main titles and subtitles do not end with a full stop. Do not preserve font, width, or vertical-scale variation from legacy raster slides; regenerate the complete raster slide whenever its title band deviates. A title-contact-sheet review is required before a deck is accepted as the visual master for another week.
